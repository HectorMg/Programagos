	...
	Renderer.start = function(){
		...
		if(this.vertices != null){
			for(var polygon = 0; polygon < this.vertices.vertices.length; polygon++){
				...
				this.canvas.context.moveTo(this.vertices.vertices[polygon][0].x +
										   this.base.transform.position.x,
										   this.vertices.vertices[polygon][0].y +
										   this.base.transform.position.y);
				
				console.log("moved To ",JSON.stringify([this.vertices.vertices[polygon][0].x +
										   this.base.transform.position.x,
										   this.vertices.vertices[polygon][0].y +
										   this.base.transform.position.y]))
				console.log(JSON.stringify(this.vertices[polygon]));
				//Make a line to the next vertex
				for(var vertex = 1; vertex < this.vertices.vertices[polygon].length; vertex++){
					this.canvas.context.lineTo(this.vertices.vertices[polygon][vertex].x +
											   this.base.transform.position.x,
											   this.vertices.vertices[polygon][vertex].y +
											   this.base.transform.position.y);
				}
	...
	Renderer.clear = function(){
		if(!this.static){
			...
			this.canvas.context.clearRect(
				this.base.transform.position.x + this.vertices.topLeft.x - 1,
				this.base.transform.position.y + this.vertices.topLeft.y - 1,
